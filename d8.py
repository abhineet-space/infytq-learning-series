...
class Game:
    ...
    def sort_players_based_on_experience(self):
        self.__players_list.sort(key=lambda exp : exp.get_experience(), reverse = True)
    ...
    def display_player_details(self):
        for each_player in self.__players_list:
            print(each_player.get_name(), end = ", ")
            print(each_player.get_experience())

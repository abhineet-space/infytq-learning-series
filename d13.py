from numpy import NaN, fromregex
...
def total_free_blocks(mem_block):
    ...
    while temp != None:
        if temp.get_data() == "Free":
            free_blocks += 1
        temp = temp.get_next()
    return ((free_blocks/total_blocks)*100)

def maximum_contiguous_free_blocks(mem_block):
    temp = mem_block.get_head()
    total_blocks = find_total_nodes(mem_block)
    free_contigous_blocks = 0
    free_list = []
    if temp.get_data() == "Free":
        free_contigous_blocks += 1
    ...
    while temp is not None:
        if temp.get_data() == "Free":
            if prev_data == "Free":
                ...
                free_list.append(free_contigous_blocks)
                free_contigous_blocks = 1
        else:
            free_list.append(free_contigous_blocks)
            free_contigous_blocks = 0
        ...
        temp = temp.get_next()
    free_list.append(free_contigous_blocks)
    max_free_contiguous_block = max(free_list)
    return (max_free_contiguous_block/total_blocks) * 100
